...
def main(name,L_Left,path):

	## Read input files
	# f_motiflength = open('./dataset/motiflength.txt', 'rb')
	# for line in f_motiflength:
	# 	W = int(line)  ## motiflength
	# f_motiflength.close()
	W = int(L_Left)
	f_sequences = open(path, 'rb')
	sequences = []
	flag = 0
	...
	f = open(name + '-predictedmotif.txt', 'wb')
	...
	np.save(name +  '-IC.npy', info[0][1:])


if __name__ == "__main__":
	main(name=sys.argv[1], L_Left=sys.argv[2], path= sys.argv[3])
