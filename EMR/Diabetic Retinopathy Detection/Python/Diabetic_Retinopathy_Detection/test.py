...
class dl_model():
    EXT_TRAIN_DATA = 'test2'
    EXT_TEST_DATA = 'test2'
    EXT_TRAIN_CSV = 'trainLabels2.csv'
    ...
    def __init__(self, argv):
        ...
        self.argv = argv
        self.BASE_PATH = 'C:\wamp64\www\dr2\Diabetic_Retinopathy_Detection\data'  # argv[0]
    ...
    def get_image_names(self,name):
        self.train_image_names_with_labels = self.get_image_name_list(os.path.join(self.BASE_PATH, self.EXT_TRAIN_CSV),
                                                                      1)  # returns a tuple
        self.test_image_names = self.get_image_name_list(os.path.join(self.BASE_PATH, self.EXT_TEST_DATA,name),
                                                         0)  # returns just names

        # print('Number of training images: {}\nNumber of testing images: {}'.format(
         #   len(self.train_image_names_with_labels[0]), len(self.test_image_names[0])))
        print(self.test_image_names)
    ...
    def execute(self,name):
        with tf.device('/cpu:0'):
            ...
            training_batch_generator = self.image_batch_generator(self.train_image_names_with_labels,
                                                                  self.GENERATOR_BATCH_SIZE, self.EXT_TRAIN_DATA)
...
if __name__ == '__main__':
    dl_model(sys.argv).execute(name=sys.argv[1])
